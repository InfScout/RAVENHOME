...
public class Interactions : MonoBehaviour
{
    ...
    void Interaction()
    {
        if (Input.GetKeyDown(KeyCode.E))
        {
            ...
            {
                TryPickupObject();
                AUDIO.GetInstance().PlaySound(AUDIO.GetInstance().blockdrop);
            }
        }
    ...
    private void throwOBJ()
    {
        if (Input.GetKeyDown(KeyCode.Mouse0) && _isHolidngObject)
        {
            ...
            Vector3 throwObject = _target.transform.forward * throwStrength + _target.transform.up * (throwStrength / 2);
